...
export const Header = ({ 
  ...
  return (
    <header className="w-full bg-white shadow-sm sticky top-0 z-50 transition-all duration-300">
      ...
      <div className="bg-[#700619] text-white py-2 px-4 sm:px-6 lg:px-8 border-b border-[#880820]">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-2 text-xs sm:text-sm">
          ...
          <div className="flex items-center gap-2 font-medium tracking-wide">
            <MapPin className="w-5 h-5 text-rose-200 shrink-0" />
            <span>Puducherry &amp; Tamil Nadu</span>
          </div>

          {/* Center: Tamil Regional Tagline */}
          <div className="font-tamil font-semibold text-sm sm:text-base tracking-wider text-amber-100 flex items-center gap-2">
          ...
          <div className="flex items-center space-x-4 text-white/90">
            <a 
              ...
              rel="noreferrer"
              aria-label="Facebook"
              className="hover:text-amber-200 transition-colors p-1 hover:bg-white/10 rounded-full"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-1.5 sm:py-2">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-7 text-sm font-semibold text-slate-800">
            {navItems.map((item) => {
              ...
              if (item.name === 'Our Services') {
                return (
                  ...
                  >
                    ...
                    >
                      <span>{item.name}</span>
                      <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${servicesDropdownOpen ? 'rotate-180 text-[#700619]' : 'text-slate-400'}`} />
                    </button>
              ...
              if (item.name === 'Resources') {
                return (
                  ...
                  >
                    ...
                    >
                      <span>{item.name}</span>
                      <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${resourcesDropdownOpen ? 'rotate-180 text-[#700619]' : 'text-slate-400'}`} />
                    </button>

                    {/* Resources Dropdown */}
                    {resourcesDropdownOpen && (
                      <div className="absolute left-0 mt-1 w-72 bg-white rounded-xl shadow-xl border border-gray-100 p-3 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                        <div className="space-y-1">
                          {item.items?.map((sub, idx) => {
                            const IconComp = sub.icon;
                            return (
              ...
              return (
                <button
                  key={item.name}
                  onClick={() => setActiveTab(item.name)}
                  className={`relative py-2 transition-colors hover:text-[#700619] ${
                    isActive ? 'text-[#700619] font-bold' : 'text-slate-700'
                  }`}
                >
                  {item.name}
                  {/* Underline indicator for active Home state (matching exact reference screenshot) */}
                  {isActive && (
          ...
          <div className="hidden sm:flex items-center">
            <button 
              ...
              className="bg-[#700619] hover:bg-[#570413] text-white px-5 sm:px-6 py-2.5 rounded-full font-semibold text-xs sm:text-sm flex items-center gap-2.5 shadow-md shadow-[#700619]/25 hover:shadow-lg hover:shadow-[#700619]/35 transition-all duration-200 active:scale-95 transform hover:-translate-y-0.5 cursor-pointer"
            >
              <Phone className="w-4 h-4 fill-white stroke-none text-white -rotate-12" />
          ...
          <div className="lg:hidden flex items-center gap-2">
            <button
              onClick={onOpenConsultation}
              className="sm:hidden bg-[#700619] text-white p-2 rounded-full shadow-md"
            ...
            <button
              ...
              className="p-2 text-slate-700 hover:text-[#700619] hover:bg-rose-50 rounded-lg transition-colors"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-gray-100 bg-white px-4 pt-3 pb-6 space-y-3 shadow-lg animate-in slide-in-from-top-4 duration-200">
